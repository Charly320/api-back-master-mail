...
export class MailService {
  ...
  async sendVerificationCode(email: string, code: string): Promise<any> {
    try {
      let templatePath = path.join(__dirname, 'templates', 'password-recovery.html');
      if (!fs.existsSync(templatePath)) {
        templatePath = path.join(process.cwd(), 'src', 'modules', 'mail', 'templates', 'password-recovery.html');
      }
      ...
      const cleanEmail = email.toLowerCase().replace(/[^a-z0-9@._+-]/g, '').trim();
      ...
      if (response.error) {
        this.logger.error(`Resend error for ${email}: ${JSON.stringify(response.error)}`);
      ...
      if (messageId) {
        ...
        this.logger.warn(`Email response for ${email} did not contain an ID. Full response: ${JSON.stringify(response)}`);
      }
      return response;
    } catch (error) {
